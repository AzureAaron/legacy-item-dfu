...
public class EnchantmentsFix extends DataFix {
	...
	public EnchantmentsFix(Schema outputSchema, boolean changesType) {
	...
	protected TypeRewriteRule makeRule() {
		...
		return this.fixTypeEverywhereTyped(
				"Enchantments Fix",
				type,
				itemStackTyped -> itemStackTyped.updateTyped(nbtTagFinder, nbtTagTyped -> nbtTagTyped.update(DSL.remainderFinder(), EnchantmentsFix::fixEnchantments)));
	}

	private static Dynamic<?> fixEnchantments(Dynamic<?> tagDynamic) {
		Optional<? extends Dynamic<?>> enchantmentsOpt = tagDynamic.get("ench")
			.asStreamOpt()
			.map(enchantments -> enchantments.map(
		...
		if (enchantmentsOpt.isPresent()) {
			tagDynamic = tagDynamic.remove("ench").set("Enchantments", (Dynamic<?>)enchantmentsOpt.get());
		}
		...
		return tagDynamic.update(
			...
			storedEnchantmentsDynamic -> DataFixUtils.orElse(
					storedEnchantmentsDynamic.asStreamOpt()
						.map(storedEnchantments -> storedEnchantments.map(
