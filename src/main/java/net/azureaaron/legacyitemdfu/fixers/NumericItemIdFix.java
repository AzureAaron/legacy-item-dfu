...
public class NumericItemIdFix extends DataFix {
	/**
	 * 1.8 Id Mappings From: https://www.minecraftinfo.com/IDList.htm
	 */
	private static final Int2ObjectMap<String> NUMERIC_ID_TO_STRING_ID = DataFixUtils.make(new Int2ObjectOpenHashMap<>(), map -> {
	...
	public NumericItemIdFix(Schema outputSchema, boolean changesType) {
		super(outputSchema, changesType);
	}

	@Override
	protected TypeRewriteRule makeRule() {
		//The id is either numeric or stringified
		Type<Either<Integer, Pair<String, String>>> oldIdType = DSL.or(
				DSL.intType(),
				DSL.named(TypeReferences.ITEM_NAME.typeName(), IdentifierNormalizingSchema.getIdentifierType())
				);
		Type<Pair<String, String>> newIdType = DSL.named(TypeReferences.ITEM_NAME.typeName(), IdentifierNormalizingSchema.getIdentifierType());
		OpticFinder<Either<Integer, Pair<String, String>>> idFieldFinder = DSL.fieldFinder("id", oldIdType);
		...
		return this.fixTypeEverywhereTyped(
				...
				itemStackTyped -> itemStackTyped.update(
						idFieldFinder, newIdType, id -> id.map(numeric -> Pair.of(TypeReferences.ITEM_NAME.typeName(), fromId(numeric)), named -> named)
						)
				);
	}

	private static String fromId(int id) {
		return NUMERIC_ID_TO_STRING_ID.get(id);
	}
}
