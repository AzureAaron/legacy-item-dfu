...
public class AttributeIdFix extends DataFix {
	...
	public AttributeIdFix(Schema outputSchema, boolean changesType) {
		super(outputSchema, changesType);
	}

	private static Dynamic<?> fixItemStack(Dynamic<?> componentsDynamic) {
		return componentsDynamic.update(
				"minecraft:attribute_modifiers",
				attributeModifiersDynamic -> attributeModifiersDynamic.update(
						"modifiers",
						modifiersDynamic -> DataFixUtils.orElse(
								modifiersDynamic.asStreamOpt().result().map(AttributeIdFix::fixModifiersCasted).map(modifiersDynamic::createList),
								modifiersDynamic)
						)
				);
	}
	...
	private static Stream<Dynamic<?>> fixModifiersCasted(Stream<?> stream) {
		return fixModifiers((Stream<Dynamic<?>>) stream);
	}

	private static Stream<Dynamic<?>> fixModifiers(Stream<Dynamic<?>> dynamicStream) {
		Map<String, Dynamic<?>> map = new Object2ObjectArrayMap<>(); //Map of attribute ids to modifier dynamics

		dynamicStream.forEach(modifierDynamic -> {
			UUID uuid = getUuidFromIntArray(modifierDynamic.get("uuid").asIntStream().toArray());
			String name = modifierDynamic.get("name").asString("");
			String idFromUuid = uuid != null ? UUID_TO_ID.get(uuid) : null;
			String idFromName = NAME_TO_ID.get(name);
			
			if (idFromUuid != null) {
				modifierDynamic = modifierDynamic.set("id", modifierDynamic.createString(idFromUuid));
				map.put(idFromUuid, modifierDynamic.remove("uuid").remove("name"));
			} else if (idFromName != null) {
				Dynamic<?> entry = map.get(idFromName);

				if (entry == null) {
					modifierDynamic = modifierDynamic.set("id", modifierDynamic.createString(idFromName));
					map.put(idFromName, modifierDynamic.remove("uuid").remove("name"));
				} else {
					double d = entry.get("amount").asDouble(0.0);
					double e = modifierDynamic.get("amount").asDouble(0.0);
					map.put(idFromName, entry.set("amount", modifierDynamic.createDouble(d + e)));
				}
			} else {
				String newId = "minecraft:" + (uuid != null ? uuid.toString().toLowerCase(Locale.ROOT) : "unknown");
				modifierDynamic = modifierDynamic.set("id", modifierDynamic.createString(newId));

				map.put(newId, modifierDynamic.remove("uuid").remove("name"));
