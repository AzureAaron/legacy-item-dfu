...
public class TheFlatteningFix extends DataFix {
	...
	public TheFlatteningFix(Schema outputSchema, boolean changesType) {
		...
		return this.fixTypeEverywhereTyped(
				...
				itemStackTyped -> {
					...
					if (itemId.isPresent()) {
						...
						int damage = stack.get("Damage").asInt(0); //Gotta love Mojang using this for both item variants and item durability
						String newId = getItem(itemId.get().getSecond(), damage);

						if (newId != null) {
							itemStackTyped = itemStackTyped.set(itemIdFinder, Pair.of(TypeReferences.ITEM_NAME.typeName(), newId));
						}

						if (DAMAGEABLE_ITEMS.contains(itemId.get().getSecond())) {
							Typed<?> nbtTagTyped = itemStackTyped.getOrCreateTyped(nbtTagFinder);
							Dynamic<?> nbtTagDynamic = nbtTagTyped.get(DSL.remainderFinder());

							nbtTagDynamic = nbtTagDynamic.set("Damage", nbtTagDynamic.createInt(damage));
							itemStackTyped = itemStackTyped.set(nbtTagFinder, nbtTagTyped.set(DSL.remainderFinder(), nbtTagDynamic));
						}
	...
	private static String getItem(@Nullable String originalName, int damage) {
		if (ORIGINAL_ITEM_NAMES.contains(originalName)) {
			...
			return flattenedId == null ? FLATTENING_MAP.get(originalName + ".0") : flattenedId;
		} else {
			return null;
		}
	}
